...
static int str2val(char* s, int base, int* pres)
{
	int res = 0, c = 0;
	while (1) {
		int v = char2dig(s[c]);
...
static int csr_load_config(void)
{
	...
	ALOGD("Reading PSR file '%s'", cfg.psrfile);
	...
	while (1) {
		...
		ipos = 0;
		while (linebuf[ipos] && linebuf[ipos] <= ' ') ipos++;
		...
		// Convert to decimal
		ipos += str2val(&linebuf[ipos],16, &v);
		if (v < 0) {
			ALOGE("Error at config file line %d, pos %d: invalid hex constant {%s}",line,ipos,linebuf);
			break;
		}
		pfkey = v;
		...
		while (1) {
			...
			while (linebuf[ipos] && !ishexdig(linebuf[ipos])) ipos++;
			if (!ishexdig(linebuf[ipos])) {
				break;
			}
		...
		switch (pfkey) {
			case CSR_PSKEY_BDADDR:
				{
					/* Override it ONLY if a valid address was provided */
					if ((vnd_local_bd_addr[0] | vnd_local_bd_addr[1] |
						vnd_local_bd_addr[2] | vnd_local_bd_addr[3] |
						vnd_local_bd_addr[4] | vnd_local_bd_addr[5]) != 0) {
		...
		ALOGD("Values: (count = %d)",opos/2);
		for (v = 0 ; v < opos; v += 2) {
			ALOGD(" [%d] = 0x%04x", v, val[v] + (val[v+1] << 8));
		}
		
		// Send the command!
		if (csr_write_pfkey( pfkey, val, opos)) {
